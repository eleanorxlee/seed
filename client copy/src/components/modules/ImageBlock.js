// Add useEffect as an import.
import React, { useState, useEffect } from "react";
import FileBase64 from "react-file-base64";

const ImageBlock = ({ _id, creator_id }) => {
  ...
  return (
    <div>
      ...
      />
      <button onClick={handleUpload}>Upload Plant Picture</button>
      {url ? <img src={`${atob(url)}`} /> : "No image uploaded!"}
    </div>
  );
};
