...
const Memories = ({ userId, setShowNavBar }) => {
  ...
  useEffect(() => {
    ...
    get("/api/stories").then((storyObjs) => {
      let storiesForUser = storyObjs.filter((storyObj) => userId === storyObj.creator_id).reverse();

      console.log(storiesForUser);
      setStories(storiesForUser);
    });
  }, []);
    ...
    <div>
      <div className="gallery-scroll">
        ...
        <div className="stories-in-memories">
          {stories.map((story) => (
            <SingleStory story={story} showCreatorName={false} />
          ))}
        </div>
      </div>
